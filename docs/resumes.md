...
`GET /resume_conditions?with_professional_roles=true` — возвращает список требований для полей нового резюме аналогичный [требованиям заполнения существующего резюме](#conditions).

В случае если передан аргумент `with_professional_roles`, поле `professional_role` становится обязательным, а поле `specialization` не обязательным. Если аргумент не передан или false, `specialization` является обязательным, а `professional_role` не обязательным
...
«подмножеством» объекта [компания/работодатель](https://api.hh.ru/openapi/redoc#tag/Informaciya-o-rabotodatele).
